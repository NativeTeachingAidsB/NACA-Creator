import { 
  ...
  type SettingsProfile, type InsertSettingsProfile, type SettingsProfileData,
...
export interface IStorage {
  ...
  // Settings Profiles
  getAllSettingsProfiles(): Promise<SettingsProfile[]>;
  getSettingsProfile(id: string): Promise<SettingsProfile | undefined>;
  getSettingsProfilesByUser(userId: string): Promise<SettingsProfile[]>;
  getSettingsProfileByCommunity(communityId: string): Promise<SettingsProfile | undefined>;
  getDefaultSettingsProfile(userId?: string): Promise<SettingsProfile | undefined>;
  getGlobalSettingsProfile(): Promise<SettingsProfile | undefined>;
  createSettingsProfile(profile: InsertSettingsProfile): Promise<SettingsProfile>;
  updateSettingsProfile(id: string, profile: Partial<InsertSettingsProfile>): Promise<SettingsProfile | undefined>;
  updateSettingsProfileData(id: string, data: Partial<SettingsProfileData>): Promise<SettingsProfile | undefined>;
  deleteSettingsProfile(id: string): Promise<boolean>;
  setDefaultSettingsProfile(id: string, userId?: string): Promise<SettingsProfile | undefined>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  // Settings Profiles
  async getAllSettingsProfiles(): Promise<SettingsProfile[]> {
    return await db.select().from(settingsProfiles).orderBy(desc(settingsProfiles.isDefault), asc(settingsProfiles.name));
  }

  async getSettingsProfile(id: string): Promise<SettingsProfile | undefined> {
    const [profile] = await db.select().from(settingsProfiles).where(eq(settingsProfiles.id, id));
    return profile || undefined;
  }

  async getSettingsProfilesByUser(userId: string): Promise<SettingsProfile[]> {
    return await db.select().from(settingsProfiles)
      .where(eq(settingsProfiles.userId, userId))
      .orderBy(desc(settingsProfiles.isDefault), asc(settingsProfiles.name));
  }

  async getSettingsProfileByCommunity(communityId: string): Promise<SettingsProfile | undefined> {
    const [profile] = await db.select().from(settingsProfiles)
      .where(eq(settingsProfiles.communityId, communityId));
    return profile || undefined;
  }

  async getDefaultSettingsProfile(userId?: string): Promise<SettingsProfile | undefined> {
    if (userId) {
      const [profile] = await db.select().from(settingsProfiles)
        .where(and(eq(settingsProfiles.userId, userId), eq(settingsProfiles.isDefault, true)));
      return profile || undefined;
    }
    const [profile] = await db.select().from(settingsProfiles)
      .where(and(eq(settingsProfiles.isGlobal, true), eq(settingsProfiles.isDefault, true)));
    return profile || undefined;
  }

  async getGlobalSettingsProfile(): Promise<SettingsProfile | undefined> {
    const [profile] = await db.select().from(settingsProfiles)
      .where(eq(settingsProfiles.isGlobal, true));
    return profile || undefined;
  }

  async createSettingsProfile(insertProfile: InsertSettingsProfile): Promise<SettingsProfile> {
    const [profile] = await db.insert(settingsProfiles)
      .values(insertProfile as typeof settingsProfiles.$inferInsert)
      .returning();
    return profile;
  }

  async updateSettingsProfile(id: string, partialProfile: Partial<InsertSettingsProfile>): Promise<SettingsProfile | undefined> {
    const [profile] = await db.update(settingsProfiles)
      .set({ ...partialProfile, updatedAt: new Date() } as Partial<typeof settingsProfiles.$inferInsert>)
      .where(eq(settingsProfiles.id, id))
      .returning();
    return profile || undefined;
  }

  async updateSettingsProfileData(id: string, dataUpdate: Partial<SettingsProfileData>): Promise<SettingsProfile | undefined> {
    const existing = await this.getSettingsProfile(id);
    if (!existing) return undefined;
    
    const mergedData: SettingsProfileData = {
      ...existing.data,
      ...dataUpdate,
      site: { ...existing.data.site, ...(dataUpdate.site || {}) },
      components: { ...existing.data.components, ...(dataUpdate.components || {}) },
      workspace: { ...existing.data.workspace, ...(dataUpdate.workspace || {}) },
      theme: { ...existing.data.theme, ...(dataUpdate.theme || {}) },
      keyboardBindings: dataUpdate.keyboardBindings ?? existing.data.keyboardBindings,
    };
  ...
  async deleteSettingsProfile(id: string): Promise<boolean> {
    const result = await db.delete(settingsProfiles).where(eq(settingsProfiles.id, id)).returning();
    return result.length > 0;
  }

  async setDefaultSettingsProfile(id: string, userId?: string): Promise<SettingsProfile | undefined> {
    // First, unset any existing default for this user/global
    if (userId) {
      await db.update(settingsProfiles)
        .set({ isDefault: false })
        .where(and(eq(settingsProfiles.userId, userId), eq(settingsProfiles.isDefault, true)));
    } else {
      await db.update(settingsProfiles)
