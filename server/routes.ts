...
import { getNacaSessionConfig, setNacaSessionConfig, clearNacaSession } from "./session";
...
import { 
  ...
  insertSettingsProfileSchema,
  settingsProfileDataSchema,
...
const videoUploadDir = path.join(process.cwd(), "uploads", "help-videos");
...
export async function registerRoutes(
  ...
  app.post("/api/help-video-candidates/upload", videoUpload.single("video"), async (req, res) => {
    try {
      if (!req.file) {
        return res.status(400).json({ error: "No video file provided" });
      }
      
      const { featureKey, testDescription } = req.body;
      if (!featureKey) {
        return res.status(400).json({ error: "Feature key is required" });
      }
      
      const videoUrl = `/uploads/help-videos/${req.file.filename}`;
      
      // Create the video candidate record
      const candidate = await storage.createVideoCandidate({
        featureKey,
        videoUrl,
        testDescription: testDescription || `Screen recording for ${featureKey}`,
        status: "pending",
      });
      ...
      console.error("Error uploading video candidate:", error);
  ...
  app.post("/api/activity-editor/negotiate", async (req, res) => {
    try {
      ...
      // Determine the best auth method - prefer api_key if we have one configured
      const supportedMethods = ['api_key', 'session', 'none'];
      let authMethod = getActiveNacaApiKey() ? 'api_key' : 'none';
      ...
      // Build auth headers based on method
      const authHeaders: Record<string, string> = {};
      if (authMethod === 'api_key' && getActiveNacaApiKey()) {
        authHeaders['Authorization'] = `Bearer ${getActiveNacaApiKey()}`;
      }
      if (subdomain) {
        authHeaders['X-Community-Subdomain'] = subdomain;
      }
      
      // Store negotiated config in session for subsequent requests
      const sessionConfig = {
        subdomain,
        authMethod,
        authHeaders,
      ...
      setNacaSessionConfig(req, sessionConfig);
      
      console.log(`[Negotiate] Session config stored: subdomain=${subdomain}, authMethod=${authMethod}`);
      ...
      res.json({
        ...
        configuration: {
          subdomain,
          authMethod,
          authHeaders: authMethod === 'api_key' ? { Authorization: 'Bearer ***' } : {},
          baseUrl: appBaseUrl,
          websocketUrl: `${wsBaseUrl}/ws/dev-sync`,
        },
        capabilities: {
  ...
  app.post("/api/activity-editor/configure-subdomain", async (req, res) => {
    try {
      ...
      // Update session config if exists
      const sessionConfig = getNacaSessionConfig(req);
      if (sessionConfig) {
        sessionConfig.subdomain = subdomain || sessionConfig.subdomain;
        if (subdomain) {
          sessionConfig.authHeaders['X-Community-Subdomain'] = subdomain;
        }
        if (communityId) {
          (sessionConfig as any).communityId = communityId;
        }
        setNacaSessionConfig(req, sessionConfig);
      }
  ...
  app.get("/api/activity-editor/session", async (req, res) => {
    try {
      const sessionConfig = getNacaSessionConfig(req);
      const hasSession = !!sessionConfig;
      const isExpired = sessionConfig?.expiresAt ? Date.now() > sessionConfig.expiresAt : false;
      ...
      res.json({
        ...
        config: sessionConfig ? {
          subdomain: sessionConfig.subdomain,
          authMethod: sessionConfig.authMethod,
          baseUrl: sessionConfig.baseUrl,
          negotiatedAt: sessionConfig.negotiatedAt,
  ...
  app.post("/api/activity-editor/session/clear", async (req, res) => {
    try {
      clearNacaSession(req);
      console.log(`[Session] Cleared NACA session`);
  ...
  app.get("/api/docs/activity-editor/bundle", async (_req, res) => {
    try {
      const bundlePath = path.resolve(process.cwd(), 'docs', 'naca-bundle');
      
      if (!fs.existsSync(bundlePath)) {
        return res.status(404).json({ error: "Bundle not found. Please generate it first." });
      }

      res.setHeader('Content-Type', 'application/zip');
      res.setHeader('Content-Disposition', 'attachment; filename=naca-integration-bundle.zip');
      ...
      archive.pipe(res);
      archive.directory(bundlePath, 'naca-integration-bundle');
  ...
  // Get bundle info
  app.get("/api/docs/activity-editor/bundle/info", async (_req, res) => {
    try {
      const bundlePath = path.resolve(process.cwd(), 'docs', 'naca-bundle');
      ...
      const fileCount = countFiles(bundlePath);
      const readmeExists = fs.existsSync(path.join(bundlePath, 'README.md'));

      res.json({
        exists: true,
        fileCount,
        hasReadme: readmeExists,
        downloadUrl: '/api/docs/activity-editor/bundle',
        contents: {
          backend: fs.existsSync(path.join(bundlePath, 'backend')),
          frontend: fs.existsSync(path.join(bundlePath, 'frontend')),
          docs: fs.existsSync(path.join(bundlePath, 'docs')),
          examples: fs.existsSync(path.join(bundlePath, 'examples'))
        }
      });
    } catch (error) {
  ...
  app.all("/api/naca-proxy/*", async (req, res) => {
    try {
      ...
      // Get session config if available (preferred for auth propagation)
      const sessionConfig = getNacaSessionConfig(req);
      const sessionSubdomain = sessionConfig?.subdomain;
      const effectiveSubdomain = sessionSubdomain || nacaSubdomain;
      
      console.log(`[NACA Proxy] ${req.method} ${targetUrl} (subdomain: ${effectiveSubdomain}, session: ${!!sessionConfig}, bootstrap: ${isCommunitiesEndpoint})`);
      ...
      // Use session-stored auth headers if available (from negotiation)
      if (sessionConfig?.authHeaders) {
        Object.assign(proxyHeaders, sessionConfig.authHeaders);
        console.log(`[NACA Proxy] Using session auth headers (method: ${sessionConfig.authMethod})`);
      } else {
      ...
      if (!isCommunitiesEndpoint) {
        ...
        if (effectiveSubdomain) {
          proxyHeaders['X-Subdomain'] = effectiveSubdomain;
          proxyHeaders['X-Community-Subdomain'] = effectiveSubdomain;
        }
      }
      ...
      if (authHeader && typeof authHeader === 'string') {
        ...
        console.log(`[NACA Proxy] Client Authorization header overrides session/API key`);
      }
      ...
      // Handle 401 Unauthorized - clear session and signal re-negotiation needed
      if (response.status === 401) {
        console.log(`[NACA Proxy] 401 Unauthorized - session may be expired`);
        if (sessionConfig) {
          clearNacaSession(req);
          console.log(`[NACA Proxy] Session cleared, client should re-negotiate`);
        }
        res.status(401).json({
          error: "Authentication failed",
          code: "AUTH_EXPIRED",
          message: "Session expired or invalid. Please re-negotiate connection.",
  ...
  // ===== NACA Integration API =====
  // High-level integration endpoints for NACA-specific functionality
  
  // Components API - Returns reusable editor components
  // GET /api/naca/components - List all components with instance counts
  app.get("/api/naca/components", async (_req, res) => {
    try {
      const allObjects = await storage.getObjectsByScreen("");
      ...
      for (const screen of screens) {
        ...
        for (const obj of objects) {
          if (obj.tags?.includes('component') || obj.classes?.includes('master-component')) {
            const existingComponent = componentsMap.get(obj.name);
            if (existingComponent) {
              existingComponent.instancesCount++;
            } else {
              componentsMap.set(obj.name, {
                id: obj.id,
                name: obj.name,
                type: obj.type,
                properties: {
  ...
  app.get("/api/naca/components/:id", async (req, res) => {
    try {
      ...
      for (const screen of screens) {
        ...
        for (const o of objects) {
          if (o.name === obj.name && (o.tags?.includes('component') || o.classes?.includes('master-component'))) {
            instancesCount++;
          }
        }
      }
  ...
  // Activity Push - Push activity updates to NACA server
  // POST /api/naca/activities/:activityId/push
  app.post("/api/naca/activities/:activityId/push", async (req, res) => {
    try {
      const { activityId } = req.params;
      const { screenId } = req.body;
      
      if (!screenId || typeof screenId !== 'string') {
        return res.status(400).json({ error: "screenId is required in request body" });
      }
      ...
      if (screen.nacaActivityId !== activityId) {
        return res.status(400).json({ 
          error: "Activity ID mismatch", 
          message: `Screen is linked to activity ${screen.nacaActivityId}, not ${activityId}` 
        });
      }
      
      if (!screen.nacaCommunityId) {
        return res.status(400).json({ error: "Screen is not linked to a NACA community" });
      }
      
      const objects = await storage.getObjectsByScreen(screenId);
      const scenes = await storage.getScenesByScreen(screenId);
      
      const sceneData = await Promise.all(scenes.map(async (scene) => {
      ...
      const activityPayload = {
        activityId,
        communityId: screen.nacaCommunityId,
        screen: {
          id: screen.id,
          title: screen.title,
          width: screen.width,
      ...
      if (!nacaBaseUrl) {
        return res.status(400).json({ error: "NACA server not configured" });
      }
      
      const targetUrl = `${nacaBaseUrl}/api/activity-editor/activities/${activityId}/sync`;
      
      const proxyHeaders: Record<string, string> = {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
        'X-Community-Id': screen.nacaCommunityId,
      };
      ...
      console.log(`[NACA Push] Pushing activity ${activityId} to ${targetUrl}`);
      ...
      if (!response.ok) {
        ...
        console.error(`[NACA Push] Server returned ${response.status}: ${errorText}`);
        return res.status(response.status).json({ 
          error: "Failed to push activity to NACA server",
          details: errorText,
        });
      }
      ...
      console.log(`[NACA Push] Successfully pushed activity ${activityId} at ${syncedAt}`);
      ...
      res.status(500).json({ 
        error: "Failed to push activity",
        details: error instanceof Error ? error.message : "Unknown error"
      });
    }
  });

  // Vocabulary Sync - Bidirectional sync with NACA dictionary
  // POST /api/naca/vocabulary/sync
  const vocabularySyncSchema = z.object({
    communityId: z.string().min(1, "communityId is required"),
    dictionaryId: z.string().min(1, "dictionaryId is required"),
    direction: z.enum(['pull', 'push', 'both']),
  });

  app.post("/api/naca/vocabulary/sync", async (req, res) => {
    try {
      const validationResult = vocabularySyncSchema.safeParse(req.body);
      ...
      const { communityId, dictionaryId, direction } = validationResult.data;
      ...
      if (direction === 'pull' || direction === 'both') {
        const pullUrl = `${nacaBaseUrl}/api/activity-editor/dictionaries/${dictionaryId}/entries`;
        console.log(`[NACA Vocab Sync] Pulling from ${pullUrl}`);
        ...
        if (pullResponse.ok) {
          const remoteEntries = await pullResponse.json();
            ...
            for (const entry of remoteEntries) {
              if (entry.word && entry.translation) {
                try {
                  ...
                  console.warn(`[NACA Vocab Sync] Failed to create vocabulary entry for "${entry.word}":`, err);
                }
              }
            }
          }
          console.log(`[NACA Vocab Sync] Pulled ${pulledCount} entries from NACA`);
        } else {
          console.error(`[NACA Vocab Sync] Pull failed with status ${pullResponse.status}`);
        }
      }
      
      if (direction === 'push' || direction === 'both') {
        const localVocabulary = await storage.getAllVocabulary();
        
        const pushUrl = `${nacaBaseUrl}/api/activity-editor/dictionaries/${dictionaryId}/entries/batch`;
        console.log(`[NACA Vocab Sync] Pushing ${localVocabulary.length} entries to ${pushUrl}`);
        
        const entriesToPush = localVocabulary.map(v => ({
          word: v.word,
        ...
        if (pushResponse.ok) {
          ...
          pushedCount = pushResult.created || entriesToPush.length;
          console.log(`[NACA Vocab Sync] Pushed ${pushedCount} entries to NACA`);
        } else {
          console.error(`[NACA Vocab Sync] Push failed with status ${pushResponse.status}`);
        }
      }
  ...
  app.get("/api/settings-profiles", async (_req, res) => {
    try {
      const profiles = await storage.getAllSettingsProfiles();
      res.json(profiles);
    } catch (error) {
      console.error("[SettingsProfiles] Error fetching profiles:", error);
      res.status(500).json({ error: "Failed to fetch settings profiles" });
    }
  });

  // Get current/active settings profile (global or default)
  app.get("/api/settings-profiles/current", async (req, res) => {
    try {
      const userId = req.query.userId as string | undefined;
      
      // Try to find user-specific default first
      if (userId) {
        const userDefault = await storage.getDefaultSettingsProfile(userId);
        if (userDefault) {
      ...
      const global = await storage.getGlobalSettingsProfile();
      if (global) {
      ...
      res.json({ 
        id: null, 
        name: "Default", 
        data: defaultSettingsProfileData,
        isDefault: true,
        isGlobal: true
      });
    } catch (error) {
      console.error("[SettingsProfiles] Error fetching current profile:", error);
      res.status(500).json({ error: "Failed to fetch current settings profile" });
    }
  });

  // Get settings profile by ID
  app.get("/api/settings-profiles/:id", async (req, res) => {
    try {
      const profile = await storage.getSettingsProfile(req.params.id);
      if (!profile) {
        return res.status(404).json({ error: "Settings profile not found" });
      }
      res.json(profile);
    } catch (error) {
      console.error("[SettingsProfiles] Error fetching profile:", error);
      res.status(500).json({ error: "Failed to fetch settings profile" });
    }
  });

  // Get settings profile by community ID
  app.get("/api/settings-profiles/community/:communityId", async (req, res) => {
    try {
      const profile = await storage.getSettingsProfileByCommunity(req.params.communityId);
      if (!profile) {
        return res.status(404).json({ error: "Community settings profile not found" });
      }
      res.json(profile);
    } catch (error) {
      console.error("[SettingsProfiles] Error fetching community profile:", error);
      res.status(500).json({ error: "Failed to fetch community settings profile" });
    }
  });
  ...
  app.post("/api/settings-profiles", async (req, res) => {
    try {
      const validatedData = insertSettingsProfileSchema.parse(req.body);
      const profile = await storage.createSettingsProfile(validatedData);
      res.status(201).json(profile);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return res.status(400).json({ error: "Validation error", details: error.errors });
      }
      console.error("[SettingsProfiles] Error creating profile:", error);
      res.status(500).json({ error: "Failed to create settings profile" });
    }
  });

  // Update settings profile
  app.patch("/api/settings-profiles/:id", async (req, res) => {
    try {
      const profile = await storage.updateSettingsProfile(req.params.id, req.body);
      if (!profile) {
        return res.status(404).json({ error: "Settings profile not found" });
      }
      res.json(profile);
    } catch (error) {
      console.error("[SettingsProfiles] Error updating profile:", error);
      res.status(500).json({ error: "Failed to update settings profile" });
    }
  });

  // Update settings profile data (merge with existing)
  app.patch("/api/settings-profiles/:id/data", async (req, res) => {
    try {
      const dataUpdate = settingsProfileDataSchema.partial().parse(req.body);
      const profile = await storage.updateSettingsProfileData(req.params.id, dataUpdate);
      if (!profile) {
        return res.status(404).json({ error: "Settings profile not found" });
      }
      res.json(profile);
      ...
      console.error("[SettingsProfiles] Error updating profile data:", error);
      res.status(500).json({ error: "Failed to update settings profile data" });
    }
  });

  // Set profile as default
  app.post("/api/settings-profiles/:id/set-default", async (req, res) => {
    try {
      const userId = req.body.userId as string | undefined;
      const profile = await storage.setDefaultSettingsProfile(req.params.id, userId);
      if (!profile) {
        return res.status(404).json({ error: "Settings profile not found" });
      }
      res.json(profile);
    } catch (error) {
      console.error("[SettingsProfiles] Error setting default profile:", error);
      res.status(500).json({ error: "Failed to set default settings profile" });
    }
  });
  ...
  app.delete("/api/settings-profiles/:id", async (req, res) => {
    try {
      const deleted = await storage.deleteSettingsProfile(req.params.id);
      if (!deleted) {
        return res.status(404).json({ error: "Settings profile not found" });
      }
      res.status(204).send();
    } catch (error) {
      console.error("[SettingsProfiles] Error deleting profile:", error);
      res.status(500).json({ error: "Failed to delete settings profile" });
    }
  });

  // Create or get global settings profile with defaults
  app.post("/api/settings-profiles/initialize-global", async (_req, res) => {
    try {
      let global = await storage.getGlobalSettingsProfile();
      if (!global) {
        global = await storage.createSettingsProfile({
      ...
      console.error("[SettingsProfiles] Error initializing global profile:", error);
      res.status(500).json({ error: "Failed to initialize global settings profile" });
    }
  });
