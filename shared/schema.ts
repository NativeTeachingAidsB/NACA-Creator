...
// Comprehensive settings storage for admin tools, theming, workspace configuration
...
// Component options (editor-specific preferences)
export const componentOptionsSchema = z.object({
  timeline: z.object({
    snapToKeyframes: z.boolean().default(true),
    snapIncrement: z.number().default(10),
    showWaveforms: z.boolean().default(false),
  }).default({}),
  canvas: z.object({
  ...
  objects: z.object({
    ...
    showBounds: z.boolean().default(true),
    transformHandleSize: z.number().default(8),
  }).default({}),
  selection: z.object({
    strokeColor: z.string().default('#3b82f6'),
    strokeWidth: z.number().default(2),
    handleColor: z.string().default('#ffffff'),
    handleBorderColor: z.string().default('#3b82f6'),
  }).default({}),
}).default({});
export type ComponentOptions = z.infer<typeof componentOptionsSchema>;

// Workspace configuration (panel layout, visibility)
export const workspaceConfigSchema = z.object({
  panelLayout: z.enum(['default', 'compact', 'expanded', 'custom']).default('default'),
  leftPanelWidth: z.number().default(280),
  rightPanelWidth: z.number().default(300),
  bottomPanelHeight: z.number().default(200),
  collapsedPanels: z.array(z.string()).default([]),
  panelOrder: z.array(z.string()).default([]),
}).default({});
export type WorkspaceConfig = z.infer<typeof workspaceConfigSchema>;

// Theme configuration (NACA-integrated)
export const themeConfigSchema = z.object({
  mode: z.enum(['light', 'dark', 'system']).default('system'),
  accentColor: z.string().default('#3b82f6'),
  fontFamily: z.string().default('system-ui'),
  communityFontFamily: z.string().nullable().default(null),
  communityFontUrl: z.string().nullable().default(null),
  fontSize: z.enum(['small', 'medium', 'large']).default('medium'),
  density: z.enum(['compact', 'cozy', 'comfortable']).default('cozy'),
  borderRadius: z.enum(['none', 'small', 'medium', 'large']).default('medium'),
  communityColors: z.object({
...
// Expertise modes for agent configuration
export const expertiseModes = [
  'ui-ux',
  'data',
  'testing',
  'creative-analysis',
  'routing',
  'domains',
  'interactive-data-driven-animation',
] as const;
export type ExpertiseMode = typeof expertiseModes[number];

// Response style modes for agent output
...
// Completion levels for agent target completion
export const completionLevels = [
  'wireframing',
  'ui-ux',
  'fully-functional-tested',
  'expert-analyzed-refactor',
] as const;
export type CompletionLevel = typeof completionLevels[number];

// Site settings (general application preferences)
export const siteSettingsSchema = z.object({
  videoHelpEnabled: z.boolean().default(true),
  showHelpTooltips: z.boolean().default(true),
  autoPlayVideos: z.boolean().default(true),
  showShortcutHints: z.boolean().default(true),
  ...
  nacaEnvironment: z.enum(['development', 'production']).default('production'),
  nacaSubdomain: z.string().default(''),
  expertise: z.enum(expertiseModes).default('ui-ux'),
  responseStyle: z.enum(responseStyles).default('brief'),
  completion: z.enum(completionLevels).default('fully-functional-tested'),
}).default({});
export type SiteSettings = z.infer<typeof siteSettingsSchema>;

// Complete settings profile schema
export const settingsProfileDataSchema = z.object({
  site: siteSettingsSchema,
  keyboardBindings: z.array(keyboardBindingSchema).default([]),
  components: componentOptionsSchema,
  workspace: workspaceConfigSchema,
  theme: themeConfigSchema,
});
export type SettingsProfileData = z.infer<typeof settingsProfileDataSchema>;

// Settings profiles table
export const settingsProfiles = pgTable("settings_profiles", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  name: text("name").notNull().default("Default"),
  userId: varchar("user_id").references(() => users.id, { onDelete: "cascade" }),
  communityId: text("community_id"), // NACA community ID for community-specific theming
  isDefault: boolean("is_default").default(false),
  isGlobal: boolean("is_global").default(false), // Shared across all users
  data: jsonb("data").$type<SettingsProfileData>().notNull(),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});

export const insertSettingsProfileSchema = createInsertSchema(settingsProfiles).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type InsertSettingsProfile = z.infer<typeof insertSettingsProfileSchema>;
export type SettingsProfile = typeof settingsProfiles.$inferSelect;

// Default settings profile data
export const defaultSettingsProfileData: SettingsProfileData = {
  site: {
  ...
  keyboardBindings: [],
  components: {
    timeline: { snapToKeyframes: true, snapIncrement: 10, showWaveforms: false },
    canvas: { showGrid: true, gridSize: 10, snapToGrid: true, showRulers: true, showGuides: true },
    objects: { showLabels: false, showBounds: true, transformHandleSize: 8 },
    selection: { strokeColor: '#3b82f6', strokeWidth: 2, handleColor: '#ffffff', handleBorderColor: '#3b82f6' },
  },
  workspace: {
    panelLayout: 'default',
    leftPanelWidth: 280,
