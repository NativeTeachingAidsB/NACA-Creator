...
export function SVGObjectExplorer({ svgContent, onClose, onImport }: SVGObjectExplorerProps) {
  return (
    <SelectionProvider>
      <SVGObjectExplorerInner svgContent={svgContent} onClose={onClose} onImport={onImport} />
    </SelectionProvider>
  );
}

function SVGObjectExplorerInner({ svgContent, onClose, onImport }: SVGObjectExplorerProps) {
  ...
  const { objects, gradients, viewBox, defsMarkup, objectMap, symbolRegistry } = useMemo(() => {
      ...
      <div className="p-3 border-b space-y-2 shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h2 className="text-base font-semibold">SVG Object Explorer</h2>
            {isAnimateExport(svgContent) && (
              <Badge className="bg-purple-500/20 text-purple-700 dark:text-purple-300 border-purple-500/30 text-[10px]">
                Adobe Animate
              </Badge>
            )}
          </div>
...
function parseSVG(svgContent: string): { 
  ...
  if (!svg) {
    return { objects: [], gradients: [], defsMarkup: '', objectMap: context.objectMap, symbolRegistry };
  }
  ...
  const isAnimate = isAnimateExport(svgContent);
  ...
  return { objects, gradients, viewBox, defsMarkup, objectMap: context.objectMap, symbolRegistry };
}
...
function parseElement(
  ...
  const displayName = animateInfo.isAnimateFormat 
  ...
  const obj: SVGObject = {
    ...
    nameSource: animateInfo.isAnimateFormat ? 'id' : naming.nameSource,
    isAutoGenerated: animateInfo.isAnimateFormat ? false : naming.isAutoGenerated,
    type,
    instanceOf,
    symbolId,
    isSymbolInstance,
