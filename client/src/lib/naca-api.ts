...
// ============================================================================
// Embeddable Components Types (NEW - per NACA Activity Editor API spec)
// ============================================================================

export interface NACAEmbedComponent {
  name: string;
  type: string;
  description: string;
  category?: string;
...
export interface NACAEmbedManifest {
  type: string;
  name: string;
  description: string;
  version: string;
...
export interface NACAEmbedTokenRequest {
  communityId: string;
  dictionaryId?: string;
  activityId?: string;
  expiresIn?: number;
...
export interface NACAEmbedToken {
  token: string;
  expiresAt: string;
  embedUrl?: string;
}

export interface NACAEmbedTokenValidation {
  valid: boolean;
  error?: string;
  communityId?: string;
...
// ============================================================================
// WebSocket Message Types (NEW - for bidirectional sync)
// ============================================================================

export type NACAWebSocketDirection = 'server-to-client' | 'client-to-server' | 'bidirectional';

export interface NACAWebSocketMessage<T = unknown> {
  type: string;
  timestamp: number;
  componentId?: string;
  data: T;
...
export interface NACAMediaUploadEvent {
  action: 'upload';
  mediaId: string;
  filename: string;
  mimeType: string;
...
export interface NACAMediaLinkEvent {
  action: 'link' | 'unlink';
  mediaId: string;
  targetId: string;
  targetType: 'game_data' | 'dictionary_entry';
}

export interface NACAMediaDeleteEvent {
  action: 'delete';
  mediaId: string;
}

export interface NACACapabilitiesUpdateEvent {
  apiVersion: string;
...
export interface NACAComponentUpdateEvent {
  componentId: string;
  data: Record<string, unknown>;
}
...
class NegotiationClient {
  ...
  private async clearBackendSession(): Promise<void> {
    try {
      await fetch('/api/activity-editor/session/clear', { method: 'POST' });
    } catch (error) {
      console.warn('[Negotiation] Failed to clear backend session:', error);
    }
  }
  
  async syncWithBackend(): Promise<boolean> {
    try {
      const response = await fetch('/api/activity-editor/session', {
        credentials: 'include',
      });
      if (!response.ok) return false;
      
      const data = await response.json();
      if (data.success && data.hasSession && !data.isExpired && data.config) {
        this.negotiatedConfig = {
          subdomain: data.config.subdomain,
          authMethod: data.config.authMethod,
          authHeaders: {},
          baseUrl: data.config.baseUrl,
        };
        console.log('[Negotiation] Synced with backend session:', data.config.subdomain);
        return true;
      }
      
      if (data.isExpired) {
        console.log('[Negotiation] Backend session expired, needs re-negotiation');
        this.clear();
      }
      return false;
    } catch (error) {
      console.warn('[Negotiation] Failed to sync with backend:', error);
      return false;
    }
  }
  
  async handleAuthError(): Promise<boolean> {
    console.log('[Negotiation] Auth error detected, attempting re-negotiation');
    this.clear();
    
    const subdomain = this.negotiatedConfig?.subdomain;
    const result = await this.negotiate({ subdomain });
    return result?.success ?? false;
  }
  
  isSessionExpired(): boolean {
    if (!this.negotiatedConfig) return true;
    const cached = localStorage.getItem('naca_negotiated_config');
...
class NACAPIClient {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 401) {
            ...
            // Check if backend signals re-negotiation needed
            try {
              const errorData = JSON.parse(errorText);
              if (errorData.needsRenegotiation) {
                console.log('[NACA API] Re-negotiation required, attempting...');
                const success = await negotiationClient.handleAuthError();
                if (success && attempt < maxRetries - 1) {
                  console.log('[NACA API] Re-negotiation successful, retrying request');
                  continue;
                }
              }
            } catch {
          ...
          throw new Error(`NACA API error (${response.status}): ${errorText}`);
        }
  ...
  // Per NACA Activity Editor API spec: /api/activity-editor/embeds
  
  async getEmbeddableComponents(): Promise<NACAEmbedComponent[]> {
    try {
      const response = await this.fetch<NACAEmbedComponent[] | { components: NACAEmbedComponent[] }>(
        '/api/activity-editor/embeds'
      );
      return Array.isArray(response) ? response : response.components || [];
    } catch (error) {
      console.warn('[NACA API] Embeds endpoint unavailable:', error);
      return [];
    }
  }
  
  async getEmbedManifest(componentType: string): Promise<NACAEmbedManifest | null> {
    try {
      return await this.fetch<NACAEmbedManifest>(
        `/api/activity-editor/embeds/${componentType}/manifest`
      );
    } catch (error) {
      console.warn('[NACA API] Embed manifest unavailable:', error);
      return null;
    }
  }
  ...
  async generateEmbedToken(
    componentType: string,
    request: NACAEmbedTokenRequest
  ): Promise<NACAEmbedToken | null> {
    try {
      return await this.fetch<NACAEmbedToken>(
        `/api/activity-editor/embeds/${componentType}/token`,
        {
          method: 'POST',
          body: JSON.stringify(request),
        }
      ...
      console.error('[NACA API] Failed to generate embed token:', error);
      return null;
    }
  }
  
  async validateEmbedToken(token: string): Promise<NACAEmbedTokenValidation> {
    try {
      return await this.fetch<NACAEmbedTokenValidation>(
        `/api/activity-editor/embeds/validate/${token}`
      );
    } catch (error) {
      console.warn('[NACA API] Embed token validation failed:', error);
