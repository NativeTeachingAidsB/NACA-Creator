...
interface TransformHandlesProps {
  ...
  onTransformEnd?: (finalTransform?: TransformPreview) => void;
...
export function TransformHandles({
  ...
  const committedTransformRef = useRef<CommittedTransform | null>(null);
  ...
  const cleanupTransform = useCallback((finalTransform?: TransformPreview) => {
  ...
  const handlePointerDown = useCallback((e: React.PointerEvent, handle: HandleType) => {
    ...
    const handleUp = (ev: PointerEvent) => {
      ...
      // Capture final transform before clearing refs
      const finalTransform: TransformPreview | undefined = pending ? { ...pending } : undefined;
  ...
  useEffect(() => {
    ...
    if (committed.type === 'rotate') {
      const rotDiff = Math.abs((rotation - (committed.rotation || 0) + 180) % 360 - 180);
      if (rotDiff < TOLERANCE) {
        setPreviewRotation(null);
        committedTransformRef.current = null;
      }
    } else if (committed.type === 'scale') {
      const widthMatch = committed.width === undefined || Math.abs(width - committed.width) < TOLERANCE;
      const heightMatch = committed.height === undefined || Math.abs(height - committed.height) < TOLERANCE;
