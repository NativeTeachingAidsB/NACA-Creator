...
import { 
  ...
  FolderOpen, BoxSelect, Ungroup, LogOut, MousePointerClick, Layers
} from "lucide-react";
...
export function ObjectContextMenu({
  ...
  return (
    ...
    >
      ...
      {/* Group/Isolation Actions - for containers with children */}
      {(hasChildren || isInIsolationMode) && (
        <>
          <Submenu label="Group Actions" icon={<Layers className="w-4 h-4" />}>
            {/* Enter Isolation Mode */}
            {hasChildren && onEnterIsolation && !isCurrentlyIsolated && (
              <MenuItem
                icon={<FolderOpen className="w-4 h-4" />}
                label="Enter Group"
                shortcut="Double-click"
                onClick={() => { onEnterIsolation(); onClose(); }}
              />
            )}
            
            {/* Exit Isolation Mode */}
            {isInIsolationMode && onExitIsolation && (
              <MenuItem
                icon={<LogOut className="w-4 h-4" />}
                label="Exit Isolation"
                shortcut="Esc"
                onClick={() => { onExitIsolation(); onClose(); }}
              />
            )}
            ...
            {hasChildren && onSelectChildren && (
              <MenuItem
                ...
                label={`Select All Inside${childCount > 0 ? ` (${childCount})` : ''}`}
                onClick={() => { onSelectChildren(); onClose(); }}
              />
            )}
