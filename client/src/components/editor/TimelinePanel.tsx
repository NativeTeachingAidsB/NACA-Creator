...
export function TimelinePanel({
  ...
  // Scroll-to-layer and flash highlight state
  ...
  // Scroll to selected layer and flash highlight
  useEffect(() => {
    if (!selectedObjectId) return;
    
    // Set highlight state for flash animation
    setRecentHighlightId(selectedObjectId);
    
    // Clear highlight after animation completes
    const timeoutId = setTimeout(() => {
      setRecentHighlightId(null);
    }, 800);
    
    // Scroll the layer row into view in both panels
    const layerRow = layerRowRefs.current[selectedObjectId];
    if (layerRow) {
  ...
  const handleToggleLock = useCallback((objectId: string) => {
    const obj = objects.find(o => o.id === objectId);
    if (obj) {
      // Select the object first to trigger scroll-to-layer
      onSelectObject(objectId);
      onUpdateObject(objectId, { locked: !obj.locked });
    }
  }, [objects, onUpdateObject, onSelectObject]);
            ...
            <div className="w-[200px] shrink-0 border-r border-border bg-muted/20 flex flex-col">
              ...
              <ScrollArea className="flex-1" ref={leftPanelScrollRef}>
                {objects.map(obj => (
                  <div
                    key={obj.id}
                    ref={el => { layerRowRefs.current[obj.id] = el; }}
                    className={cn(
                      "h-6 flex items-center gap-1 px-1 cursor-pointer hover:bg-accent/20 border-b border-border/50 transition-all duration-300",
                      selectedObjectId === obj.id && "bg-accent/40",
                      recentHighlightId === obj.id && "ring-2 ring-primary ring-inset animate-pulse bg-primary/20"
                    )}
                  ...
                  >
                    <Button
                      ...
                      data-testid={`layer-visibility-left-${obj.id}`}
                    >
                      {obj.visible !== false ? (
                    ...
                    <Button
                      ...
                      data-testid={`layer-lock-left-${obj.id}`}
                    >
                      {obj.locked ? (
                        <Lock className="h-3 w-3 text-amber-500" />
                      ) : (
