...
interface SelectedScreenInfo {
  id: string;
  title: string;
  nacaActivityId?: string | null;
  nacaCommunityId?: string | null;
}

interface AttributeEditorProps {
  selectedObject: GameObject | null;
  selectedObjects?: GameObject[];
  selectedScreen?: SelectedScreenInfo | null;
...
function hashToColor(str: string | undefined | null): string {
  if (!str) return 'hsl(220, 70%, 50%)';
  let hash = 0;
...
const ACTIVITY_METADATA_FIELDS = [
  { key: 'activityDescriptionClean', label: 'Description', icon: FileText },
  { key: 'activityGenerator', label: 'Generator', icon: Settings },
  { key: 'interactionType', label: 'Interaction', icon: MousePointer },
  { key: 'difficultyLevel', label: 'Difficulty', icon: LayersIcon },
  { key: 'tags', label: 'Tags', icon: Tag },
  { key: 'mediaIncluded', label: 'Media', icon: ImageIcon },
  { key: 'learningFocus', label: 'Focus', icon: Hash },
  { key: 'activityModel', label: 'Model', icon: Database },
  { key: 'mechanicRule', label: 'Mechanic', icon: Zap },
] as const;

interface SelectedActivityPropertiesProps {
  communityId?: string | null;
  activityId?: string | null;
  screenTitle?: string;
}

function SelectedActivityProperties({ communityId, activityId, screenTitle }: SelectedActivityPropertiesProps) {
  const { isConfigured } = useNacaApiConfig();
  const { data: communities } = useNacaCommunities();
  
  const hasValidIds = !!communityId && !!activityId;
  const isApiAvailable = isConfigured && hasValidIds;
  ...
  const { data: activityItems, isLoading: isLoadingItems } = useNacaActivityItems(
    communityId || '',
    activityId || '',
    { enabled: isApiAvailable }
  );
  ...
  const isLoading = isApiAvailable && (isLoadingActivity || isLoadingItems);
  ...
  }, [activityDetails?.metadata, activityDetails?.settings]);
  ...
  if (!hasValidIds) {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center">
        ...
        <span className="text-sm text-muted-foreground">No activity attached</span>
      </div>
    );
  }
  ...
  if (!isConfigured) {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center">
        ...
        <span className="text-sm text-muted-foreground">NACA not configured</span>
        <span className="text-xs text-muted-foreground mt-1">Configure NACA to view activity details</span>
      </div>
    );
  }
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center p-8">
        <AvatarThrobber 
          fallback={community?.name || "Activity"} 
          src={community?.logoUrl || undefined}
          size="lg"
          isLoading={true}
          accentColor={accentColor}
        />
        <span className="text-sm text-muted-foreground mt-3">Loading activity...</span>
      </div>
    );
  }
  ...
  if (!activityDetails) {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center">
        <Globe className="h-8 w-8 text-muted-foreground mb-2" />
        <span className="text-sm text-muted-foreground">Activity not found</span>
      </div>
    );
  }
  ...
  return (
    <ScrollArea className="h-full">
      <div className="p-4 space-y-4">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <span 
                  ...
                  data-testid="activity-community-name"
                >
              ...
              <p className="text-base font-medium mt-1" data-testid="activity-name">
              ...
              <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
                ...
                {activityDetails.entryCount !== undefined && activityDetails.entryCount > 0 && (
                  <span>{activityDetails.entryCount} entries</span>
                )}
                {activityDetails.isPublished !== undefined && (
                  <Badge 
                    variant={activityDetails.isPublished ? "default" : "outline"} 
                    className="text-[10px] px-1.5 py-0"
                  >
                    {activityDetails.isPublished ? "Published" : "Draft"}
                  </Badge>
                )}
              </div>
            </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-2">
            {ACTIVITY_METADATA_FIELDS.map(field => {
              ...
              return (
                <div key={field.key} className="flex flex-col space-y-0.5">
                  ...
                  <span 
                    ...
                    title={value}
                    data-testid={`activity-metadata-${field.key}`}
                  >
                    {value}
                  </span>
                </div>
        ...
        {activityItems?.items && activityItems.items.length > 0 && (
          <>
            <Separator />
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="secondary" className="text-[10px]">
                  {activityItems.items.length} items
                </Badge>
              </div>
              
              <div className="space-y-1.5 max-h-48 overflow-y-auto">
                {activityItems.items.slice(0, 10).map((item, idx) => (
                  <div 
                    key={item.id || idx}
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      {item.language && (
                        <p className="font-medium truncate">{item.language}</p>
                      )}
                      {item.english && (
                        <p className="text-muted-foreground truncate">{item.english}</p>
                      )}
                      {!item.language && !item.english && item.category && (
                ...
                {activityItems.items.length > 10 && (
                  <p className="text-[10px] text-muted-foreground text-center py-1">
                    +{activityItems.items.length - 10} more items
                  </p>
                )}
              </div>
            </div>
        ...
        {(activityDetails.createdAt || activityDetails.updatedAt || activityDetails.createdBy) && (
          <>
            <Separator />
            <div className="space-y-2 text-xs text-muted-foreground">
              {activityDetails.createdBy && (
                <div className="flex items-center gap-2">
                  ...
                  <span>Created by {activityDetails.createdBy}</span>
                </div>
              )}
              {activityDetails.createdAt && (
                <div className="flex items-center gap-2">
                  <Calendar className="w-3 h-3" />
                  <span>Created {new Date(activityDetails.createdAt).toLocaleDateString()}</span>
                </div>
              )}
              {activityDetails.updatedAt && (
                <div className="flex items-center gap-2">
                  <Calendar className="w-3 h-3" />
                  <span>Updated {new Date(activityDetails.updatedAt).toLocaleDateString()}</span>
                </div>
              )}
            </div>
          </>
        ...
        {screenTitle && (
          <>
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <FileText className="w-3 h-3" />
              <span>Attached to screen: <span className="font-medium text-foreground">{screenTitle}</span></span>
            </div>
          </>
        )}
      </div>
...
export function AttributeEditor({
  ...
  const hasAttachedActivity = selectedScreen?.nacaCommunityId && selectedScreen?.nacaActivityId;
      ...
      <div className="h-full flex flex-col">
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "properties" | "styles")} className="h-full flex flex-col">
          <div className="px-4 pt-4 border-b border-border">
            <TabsList className="w-full grid grid-cols-2 h-8">
              <TabsTrigger value="properties" className="text-xs" data-testid="tab-properties">
                {hasAttachedActivity ? "Activity" : "Properties"}
              </TabsTrigger>
          ...
          <TabsContent value="properties" className="flex-1 m-0 overflow-hidden">
            {hasAttachedActivity ? (
              <SelectedActivityProperties 
                communityId={selectedScreen.nacaCommunityId!}
                activityId={selectedScreen.nacaActivityId!}
                screenTitle={selectedScreen.title}
              />
            ) : (
              <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground text-sm p-4 text-center h-full">
                <MousePointer className="h-8 w-8 mb-3 opacity-50" />
                <p>Select an object to edit its attributes</p>
                <p className="text-xs mt-1 opacity-70">Or attach an activity to view its properties</p>
              </div>
            )}
          </TabsContent>
