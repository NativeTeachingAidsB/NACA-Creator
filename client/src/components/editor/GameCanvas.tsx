...
export function GameCanvas({
  ...
  // Committed transform holds the final scale/rotation values until mutation completes (prevents snap-back)
  const committedTransformRef = useRef<{
    objectId: string;
    width?: number;
    height?: number;
    x?: number;
  ...
  useGSAP(() => {
    ...
    objects.forEach(obj => {
      ...
      // Skip GSAP for objects being transformed (scale/rotate) or with pending committed transform
      // This prevents snap-back where GSAP animates to old values before mutation completes
      const isBeingTransformedByGsap = transformingObjectId === obj.id;
      const hasCommittedTransform = committedTransformRef.current?.objectId === obj.id;
      
      if (isBeingDraggedOrCommitted || isBeingTransformedByGsap || hasCommittedTransform) {
  ...
  useEffect(() => {
    ...
    // Handle committed transform (scale/rotation) - clear when database matches
    const committedTransform = committedTransformRef.current;
    if (committedTransform) {
      const obj = objects.find(o => o.id === committedTransform.objectId);
      if (obj) {
        const TOLERANCE = 1;
        const widthMatches = committedTransform.width === undefined || 
          Math.abs(obj.width - committedTransform.width) < TOLERANCE;
        const heightMatches = committedTransform.height === undefined || 
          Math.abs(obj.height - committedTransform.height) < TOLERANCE;
        const rotationMatches = committedTransform.rotation === undefined || 
          Math.abs((obj.rotation ?? 0) - committedTransform.rotation) < TOLERANCE;
        const xMatches = committedTransform.x === undefined || 
          Math.abs(obj.x - committedTransform.x) < TOLERANCE;
        const yMatches = committedTransform.y === undefined || 
          Math.abs(obj.y - committedTransform.y) < TOLERANCE;
        
        if (widthMatches && heightMatches && rotationMatches && xMatches && yMatches) {
          // Database values match committed transform, safe to clear
  ...
  return (
    <div className="h-full flex flex-col bg-background relative overflow-hidden">
        ...
        >
          ...
          >
            ...
            {sortedObjects.map(obj => {
              ...
              // Priority: active transformPreview > committedTransform (pending mutation) > database values
              const isBeingTransformed = transformingObjectId === obj.id && transformPreview;
              const hasCommittedTransform = committedTransformRef.current?.objectId === obj.id;
              const committedTransform = hasCommittedTransform ? committedTransformRef.current : null;
              ...
              if (isBeingTransformed && transformPreview) {
                // Active drag - use preview values
              ...
              } else if (committedTransform) {
                // After drag released but before mutation completes - use committed values
                if (committedTransform.x !== undefined) renderX = committedTransform.x;
                if (committedTransform.y !== undefined) renderY = committedTransform.y;
                if (committedTransform.width !== undefined) renderWidth = committedTransform.width;
                if (committedTransform.height !== undefined) renderHeight = committedTransform.height;
                if (committedTransform.rotation !== undefined) renderRotation = committedTransform.rotation;
              }
                  ...
                  {(isSelected || isIsolated) && !isPreviewMode && !obj.locked && (
                    <TransformHandles
                      ...
                      onTransformEnd={(finalTransform) => {
                        ...
                        // Store final transform values in committedTransformRef before clearing state
                        // This prevents snap-back by keeping the visual state until mutation completes
                        // Note: finalTransform comes directly from TransformHandles, not from React state,
                        // so it's guaranteed to have the latest values even if RAF hasn't updated state yet
                        if (finalTransform) {
      ...
      {contextMenuPos && selectedObjectId && (() => {
        ...
        const isGroup = selectedObj.type === 'group' || selectedObj.type === 'frame';
        const hasChildrenTag = selectedObj.tags?.includes('has-children') ?? false;
        const hasChildren = isGroup || hasChildrenTag;
        
        const childObjects = hasChildren ? objects.filter(o => {
          const parentTag = o.tags?.find(t => t.startsWith('parent-'));
          return parentTag === `parent-${selectedObj.customId}` || 
                 o.tags?.includes(`child-of-${selectedObj.id}`) ||
                 (selectedObj.customId && o.classes?.includes(`symbol-${selectedObj.customId?.toLowerCase()}`));
        }) : [];
        const childCount = childObjects.length;
        
        const handleEnterIsolation = () => {
          onIsolateObject(selectedObjectId);
          setContextMenuPos(null);
        };
        ...
        const handleSelectChildren = () => {
          if (childObjects.length > 0) {
            ...
            childObjects.forEach(child => {
              if (!selectedObjectIds.includes(child.id)) {
                selectedObjectIds.push(child.id);
              }
            });
            if (childObjects.length === 1) {
              onSelectObject(childObjects[0].id);
            }
          }
          setContextMenuPos(null);
        };
        ...
        const handleUngroup = () => {
          toast({
            title: "Ungroup",
            description: `Ungrouping "${selectedObj.name}" would release ${childCount} child object${childCount !== 1 ? 's' : ''}.`,
          });
          setContextMenuPos(null);
        };
        ...
        return (
          <ObjectContextMenu
            ...
            objectType={selectedObj.type}
            isVisible={selectedProps.visible}
            hasClipboard={!!clipboard}
            isMasterComponent={isMasterComp}
            isInstance={isInst}
            hasChildren={hasChildren}
            isInIsolationMode={!!isolatedObjectId}
            isCurrentlyIsolated={isolatedObjectId === selectedObjectId}
            ...
            onEnterIsolation={handleEnterIsolation}
