...
interface DevSyncContextValue {
  ...
  sendComponentUpdate: (componentId: string, data: Record<string, unknown>) => void;
}
...
export function DevSyncProvider({ children, initialComponentId, autoConnect = false }: DevSyncProviderProps) {
  ...
  const handleNacaMessage = useCallback((message: DevSyncMessage) => {
    ...
    switch (type) {
      ...
      case "componentUpdate":
        console.log("[DevSync] Component update received:", payload?.componentId, payload?.data);
        const componentHandlers = messageHandlersRef.current.get("component_update");
        if (componentHandlers) {
          componentHandlers.forEach(handler => handler(payload));
        }
        const componentHandlersCamel = messageHandlersRef.current.get("componentUpdate");
        if (componentHandlersCamel) {
          componentHandlersCamel.forEach(handler => handler(payload));
        }
        break;
    }
  }, [queryClient]);
  ...
  const sendComponentUpdate = useCallback((targetComponentId: string, data: Record<string, unknown>) => {
    send({
    ...
    console.log("[DevSync] Component update sent:", targetComponentId, data);
  }, [send]);
