...
export function useUpdateGameObject() {
  ...
  return useMutation({
    ...
    onMutate: async (variables) => {
      ...
      await queryClient.cancelQueries({ queryKey: ["gameObjects"] });
      
      const allQueries = queryClient.getQueriesData<GameObject[]>({ queryKey: ["gameObjects"] });
      const previousData: Record<string, GameObject[]> = {};
      let objectScreenId: string | undefined;
      
      allQueries.forEach(([queryKey, data]) => {
        if (data) {
          const screenId = queryKey[1] as string;
          previousData[screenId] = data;
          
          const objectIndex = data.findIndex(obj => obj.id === id);
          if (objectIndex !== -1) {
            objectScreenId = screenId;
            const updatedObjects = [...data];
            updatedObjects[objectIndex] = { 
              ...data[objectIndex], 
              ...updates,
              metadata: updates.metadata !== undefined 
                ? updates.metadata as GameObject['metadata']
                : data[objectIndex].metadata
            } as GameObject;
            queryClient.setQueryData<GameObject[]>(queryKey, updatedObjects);
          }
        }
      });
      
      return { previousData, objectScreenId };
    },
    onError: (_err, _variables, context) => {
      if (context?.previousData) {
        Object.entries(context.previousData).forEach(([screenId, data]) => {
          queryClient.setQueryData<GameObject[]>(["gameObjects", screenId], data);
        });
      }
    },
    onSettled: (_data, _error, _variables, context) => {
      if (context?.objectScreenId) {
        queryClient.invalidateQueries({ queryKey: ["gameObjects", context.objectScreenId] });
      }
    },
  });
}
